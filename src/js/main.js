  ...
  try {
    ...
    zip
      ...
      .then(function (zipFile) {
        ...
        const plistPath = Object.keys(zipFile.files).find(
          (fileName) =>
...
function startEditor(parsedPlist, zipFile) {
  ...
  const SupportsDocumentBrowser =
    parsedPlist["UISupportsDocumentBrowser"] || false;
  const FileSharingEnabled = parsedPlist["UIFileSharingEnabled"] || false;
  const SupportsOpeningDocumentsInPlace =
    parsedPlist["LSSupportsOpeningDocumentsInPlace"] || false;
  ...
  console.log(
    ...
    SupportsOpeningDocumentsInPlace
  );
  ...
  const iconFile = Object.keys(zipFile.files).find(
    (fileName) =>
      fileName.startsWith(`Payload/`) && fileName.includes(`.app/${iconName}`)
  );
...
document.addEventListener("submit", (event) => {
  ...
  try {
    ...
    if (validForm) {
      ...
      parsedPlistGlobal["CFBundleIdentifier"] =
        document.getElementById("bundle-identifier").value;
      parsedPlistGlobal["CFBundleName"] =
        document.getElementById("bundle-name").value;
      parsedPlistGlobal["CFBundleDisplayName"] =
        document.getElementById("display-name").value;
      parsedPlistGlobal["CFBundleVersion"] =
        document.getElementById("bundle-version").value;
      parsedPlistGlobal["CFBundleShortVersionString"] = document.getElementById(
        "short-version-string"
      ).value;
      parsedPlistGlobal["MinimumOSVersion"] =
        document.getElementById("min-ios-version").value;
      ...
      const payloadFolder = Object.keys(zipFileGlobal.files).find(
        (fileName) =>
          fileName.startsWith("Payload/") &&
          fileName.endsWith(".app/Info.plist")
      );
